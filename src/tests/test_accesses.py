...
def test_create_access(test_app, monkeypatch):
    ...
    response = test_app.post("/access/", data=json.dumps(test_request_payload))
...
def test_create_access_invalid_json(test_app):
    response = test_app.post("/access/", data=json.dumps({"device": 1}))
    assert response.status_code == 422

    response = test_app.post("/access/", data=json.dumps({**REPLY_PAYLOAD}))
    assert response.status_code == 422
...
def test_get_access(test_app, monkeypatch):
    ...
    response = test_app.get("/access/1")
...
def test_get_access_incorrect_id(test_app, monkeypatch):
    ...
    response = test_app.get("/access/999")
    assert response.status_code == 404
    assert response.json()["detail"] == "Entry not found"

    response = test_app.get("/access/0")
    assert response.status_code == 422
...
def test_update_access(test_app, monkeypatch):
    ...
    response = test_app.put("/access/1/", data=json.dumps(test_update_data))
    assert response.status_code == 200
...
def test_remove_access(test_app, monkeypatch):
    ...
    response = test_app.delete("/access/1/")
    assert response.status_code == 200
    assert {k: v for k, v in response.json().items() if k != 'created_at'} == test_data
...
def test_remove_access_incorrect_id(test_app, monkeypatch):
    ...
    response = test_app.delete("/access/999/")
    assert response.status_code == 404
    assert response.json()["detail"] == "Entry not found"

    response = test_app.delete("/access/0/")
    assert response.status_code == 422
