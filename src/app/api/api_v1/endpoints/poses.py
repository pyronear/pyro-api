...
async def create_pose(
    ...
    camera = cast(Camera, await CameraCRUD(session=session).get(payload.camera_id, strict=True))

    if token_payload.organization_id != camera.organization_id and UserRole.ADMIN not in token_payload.scopes:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Access forbidden.")

    db_pose = await PoseCRUD(session=session).create(payload)
...
async def get_pose(
    ...
    pose = cast(Pose, await PoseCRUD(session=session).get(pose_id, strict=True))
...
async def update_pose(
    ...
    db_pose = await PoseCRUD(session=session).update(pose_id, payload)
    return PoseRead(**db_pose.model_dump())
...
async def delete_pose(
    ...
    await PoseCRUD(session=session).delete(pose_id)
...
async def list_pose_masks(
    ...
    rows = await OcclusionMaskCRUD(session=session).get_by_pose(pose_id)
