...
class DetectionCreate(Azimuth):
    camera_id: Annotated[int, Field(..., gt=0)]
    pose_id: Annotated[int | None, Field(None, gt=0)]
    bucket_key: str
    bboxes: Annotated[
        str,
        Field(
...
class DetectionUrl(BaseModel):
    url: Annotated[str, Field(description="temporary URL to access the media content")]
